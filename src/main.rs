...
async fn run_preempt_loop(ctx: Context, args: Arc<Arguments>, all: &Vec<Server>) {
    ...
    loop {
        ...
        match client.preempt_leader(ctx.clone()).await {
            Ok(leader) => {
...
async fn run_reconfigure_loop(ctx: Context, args: Arc<Arguments>, all: &Vec<Server>) {
    ...
    loop {
        ...
        let start = Instant::now();
        match client.change_config(ctx.clone(), new.clone()).await {
            Ok(_) => {
...
async fn run_client_loops(
    ...
    let all = join4(
        run_put_loop(args.clone(), &all).instrument(info_span!("put")),
        run_preempt_loop(ctx.clone(), args.clone(), &all).instrument(info_span!("preempt")),
        run_validate_loop(args.clone(), diagnostics).instrument(info_span!("validate")),
        run_reconfigure_loop(ctx.clone(), args.clone(), &all).instrument(info_span!("reconfigure")),
    )
    .map(|_| Result::<(), Never>::Ok(()));

    match ctx.wrap(all).await {
        Ok(_) => Ok(()),
        Err(context::Error::ContextDone) => Ok(()),
        Err(context::Error::Inner(e)) => Err(e),
    }
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let signal_handler = async move {
        ...
        // Client loops and servers all hang on this context (or children). Canceling
