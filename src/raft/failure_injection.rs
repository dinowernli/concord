...
mod tests {
    ...
    // TODO(dino) - DONOTMERGE - extract some common bits out of the tests
    ...
    async fn test_no_failures() {
        let failure_options = Arc::new(Mutex::new(FailureOptions::no_failures()));
        let service = FakeKeyValue {};
        let mut server_handle = TestRpcServer::run(KeyValueServer::new(service)).await;
        let server = server_handle.as_server().unwrap().clone();
        let channel = get_channel(&server).await;

        let client_name = "client-for-testing".to_string();
        let channel_info = ChannelInfo::from_client(client_name.clone(), server.clone());
        let middleware: FailureInjectionMiddleware<Channel> =
            FailureInjectionMiddleware::new(channel, failure_options.clone(), channel_info);
        let mut client = KeyValueClient::new(middleware);
    ...
    async fn test_probability_failure() {
        ...
        failure_options.lock().await.failure_probability = 1.0;
        ...
        // Corrected capitalization of "Unavailable"
        assert_eq!(
            result.unwrap_err().to_string(),
            format!(
                "status: Unavailable, message: \"error injected in channel {} -> {}\", details: [], metadata: MetadataMap {{ headers: {{}} }}",
    ...
    async fn test_latency_injection() {
        ...
        {
            let mut opts = failure_options.lock().await;
